...
def calc_travel_times_fmm(oneD_vel_model_z_df, array_centre_xyz, src_xyz, extent_xy_m=[3000.,3000.], dxyz=[1.,1.,1.]):
    """Get theoretical P and S travel times with hypocentral distance from source (using Eikonal method).
    station_xyz_coords
    Inputs:
    oneD_vel_model_z_df - Pandas DataFrame of shape (z_extent, 3) with the columns corresponding to: depth, vp, vs.
    ...
    dy = dxyz[1] # Grid spacing in x dir
    dz = dxyz[2] # Grid spacing in z dir
    vel_model_arr_P, vel_model_arr_S, vel_model_x_labels, vel_model_y_labels, vel_model_z_labels = read_1D_vel_model_for_fmm(oneD_vel_model_z_df, extent_xy_m=extent_xy_m, dxyz=dxyz)
    
    # Calculate travel times for all points in the grid:
    # Setup array for masking/showing where source is:
    phi = -np.ones(vel_model_arr_P.shape)
    val, x_idx = find_nearest(vel_model_x_labels, src_xyz[0])
    val, y_idx = find_nearest(vel_model_y_labels, src_xyz[1])
    val, z_idx = find_nearest(vel_model_z_labels, src_xyz[2])
    phi[x_idx, y_idx, z_idx] = 1. # Set source location:

    # Calculate travel times array for the stations:
    ...
    # Find locations of stations within grid, and retreive associated travel times:
    trav_times_P = []
    trav_times_S = []
    for index, row in station_xyz_coords_df.iterrows():
        val, x_idx = find_nearest(vel_model_x_labels, row['x_m'])
        val, y_idx = find_nearest(vel_model_y_labels, row['y_m'])
        val, z_idx = find_nearest(vel_model_z_labels, row['z_m'])
        trav_times_P.append(trav_times_grid_P[x_idx,y_idx,z_idx])
        trav_times_S.append(trav_times_grid_S[x_idx,y_idx,z_idx])
    trav_times_P = np.array(trav_times_P)
    trav_times_S = np.array(trav_times_S)
    
    return trav_times_P, trav_times_S
...
if __name__ == "__main__":
    ...
    oneD_vel_model_z_df = pd.DataFrame({'depth': np.arange(0,2100,1.), 'vp': 3500*np.ones(2100), 'vs': 2000*np.ones(2100)})
